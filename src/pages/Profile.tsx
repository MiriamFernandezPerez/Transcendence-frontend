						...
						<div className="grid gap-4 lg:hidden">
							{profileData.history.map((match) => (
								<div key={match.id} className="glass-panel p-1.5 flex flex-col gap-3 relative overflow-hidden">
									...
									<div className={`absolute left-0 top-0 bottom-0 w-1 ${match.result === 'win' ? 'bg-success' : 'bg-danger'}`}></div>
									...
									<div className="flex justify-evenly items-center">
										{/* Badge Result */}
										<span className={`px-3 py-1 rounded text-xs font-black uppercase tracking-widest ${match.result === 'win'
								...
								<tbody className="divide-y divide-white/5">
									{profileData.history.map((match) => (
										<tr key={match.id} className="hover:bg-white/5 transition-colors text-center relative group">
											<td className={`absolute left-0 top-0 bottom-0 w-1 transition-all group-hover:w-1.5 ${match.result === 'win' ? 'bg-success' : 'bg-danger'}`}>
											</td>
